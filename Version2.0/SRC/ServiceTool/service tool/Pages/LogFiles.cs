...
namespace service_tool
{
    public partial class LogFiles : UserControl
    {
        ...
        private string localLogStart = "EXTARO300 ENT";//System Type :Start Mode
        ...
        private string ErrorCodeAnalysis(string errResponse)
        {
            ...
            //13 Helios driving voltage error
            ...
            //14 Helios illumination setting error
            ...
            //15 Helios over temperature warning
            ...
            //17 Helios temperature control error
            ...
            //19 CCU EVR communication error
            ...
            //20 CAN communication to OPMI error
            ...
            //21 CAN communication to Helios error
            ...
            //22 CAN communication to CCU error
        ...
        private void SaveAllErrorLog(int errInfoCount)
        {
            string errCmd;
            string errResponse = "";
            int index = 0;
                ...
                if (fileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if ( logList[0].Equals(oldlocallogpeak) || (logList[0].Equals(localLogPeak)) || (logList[0].Equals(localLogStart) || logList[0].Equals(localNotAvailable)))
                    {
                        ...
                        if (logList[1].Equals(localErrLog))//"Error Log"
                        {
                            ...
                            if (logList[0].Equals(oldlocallogpeak))
                            {
                                errInfoCount = Convert.ToInt32(logList[2]);
                            }
                            if (logList[0].Equals(localLogPeak))
                            {
                                 errInfoCount = Convert.ToInt32(logList[6]);
                            }
                            ...
                            else if (logList[0].Equals(localLogStart))
                            {
                ...
                if (fileDialog.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if ((logList[0].Equals(oldlocallogpeak)) || (logList[0].Equals(localLogPeak)) || (logList[0].Equals(localLogStart) || logList[0].Equals(localNotAvailable)))
                    {
                        if (logList[1].Equals(localOperLog))
                        {
                            ...
                            if (logList[0].Equals(localLogPeak))
                            {
                                 operInfoCount = Convert.ToInt32(logList[6]);
                            }
                            //added end
                            //20180115
                            if (logList[0].Equals(localLogPeak))
